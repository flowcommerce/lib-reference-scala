...
func processPaymentMethods() {
	...
	for _, pm := range common.PaymentMethods() {
		...
		instances = append(instances, Instance{
			Name:  pm.Id,
			Value: fmt.Sprintf("PaymentMethod(id = \"%s\", `type` = %s, name = \"%s\", images = %s, regions = %s, capabilities = %s)", pm.Id, typ, pm.Name, images, scalaArrayQuoted(pm.Regions), scalaArrayQuoted(pm.Capabilities)),
		})
	}
